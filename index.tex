...
\begin{center}
\pandocbounded{\includegraphics[keepaspectratio]{images/MathematicsBookCover.jpg}}
\end{center}
...
first --- although not necessary(\textbf{RECOMMENDED}). The rest can be
...
\item
  ...
  \tightlist
  \item
    \(𝐴\) and \(𝐵\) are not equal, since \(𝐴\) null set---without
    elements while \(B\) is a set that has a null set as an element.
    Thus, \(𝐴 ≠ B\).
  \item
    \(𝐴\) and \(𝐶\) are equal, since \(𝐴\) is a null set---without
    elements and \(C\) is also set that has no elements in it. Thus
    \(𝐴 = C\).
  \item
    \(𝐵\) and \(𝐶\) are not equal, since 𝐵 is a \emph{set that has a
    null set} as an element, while \(C\) is a null set . Thus,
    \(𝐵 ≠ 𝐶\).
  \end{enumerate}
\item
  As for the cardinalities: \[
  |A| = |C| = 0
  \] \[
  |B| = 1
  \]
...
A \textbf{subset} can be defined as a set within a set. As per the name
suggests, it a portion of a set, and it is denoted by \(⊆\). Formally,
we say

\begin{description}
\item[SUBSET]
The set \(X\) is the \emph{subset} of \(Y\) if every element in \(X\) is
also an element of \(Y\).
\end{description}

Suppose you have three sets \(𝐴 = \{2,4,6,8,10\}\) ; \(𝐵 = \{2,6,8\}\)
and \(𝐶 = \{2,6\}\). \(C\) is the subset of \(𝐴\) and \(𝐵\) since \(2\)
\& \(6\) are in both the sets \(𝐴\) and \(B\),that is \(C⊆𝐴\) and
\(𝐶 ⊆𝐵\).

Set \(𝐵\) is a subset of \(𝐴\) but not of \(𝐶\). That is \(B⊆𝐴\) and
\(𝐵 ⊈𝐶\). And lastly \(𝐴\) is not a subset of either sets.
...
o \textbf{\emph{Integers}}: all set of numbers without a decimal. These
are all set of integers which are either positive or negative.
Represented by the symbol \(ℤ\). denoted by
\[ℤ = \{…,−3,−2,−1,0,1,2,3,…\}\]. o \textbf{\emph{Natural Numbers}}: all
positive integers EXCEPT 0. Represented by the symbol \(ℕ\). denoted by
\[ℕ = \{1,2,3,…\}\]. o \textbf{\emph{Whole Numbers}}: all positive
integers INCLUDING \(0\), denoted by \[ℕ_0=\{0,1,2,3, …\}\]. o
\textbf{\emph{Rational Numbers}}: Represented by the symbol \(ℚ\). All
fractions of the form \[ℚ=\{ \frac{𝑎}{𝑏} | 𝑎,𝑏 ∈ ℤ ,𝑏≠ 0 \}\]. o
\textbf{\emph{Irrational numbers}}: numbers that are thought not to be
rational are said to be irrational.

o \textbf{\emph{Imaginary Numbers}}: imagined numbers that may not exist
where \(\sqrt{−1}=𝑖\).

o \textbf{\emph{Real Numbers}}: all numbers that can be quantified and
thought not to be imaginary. These numbers are made up by both rational
and irrational numbers Represented by the symbol \(ℝ\).

o \textbf{\emph{Complex Numbers}}: a combination of both real and
imaginary numbers. Represented by the symbol \(ℂ\) and denoted as
\[ℂ = \{𝑎 + 𝑏𝑖 | 𝑎,𝑏 ∈ ℝ ;i = \sqrt{−1} \}\].
...
A \textbf{\emph{Union}} contains all elements of sets, denoted by the
symbol \(∪\) and keyword \textbf{or} is formalized as
\[S \text{or} T=S∪𝑇=\{𝑎,𝑏,𝑐,1,2\}\] the notation \(S ∪ 𝑇\) is called
\(𝑆\) union \(𝑇\) which is equivalent to \(𝑆\) 𝑜𝑟\(𝑇\).

\textbf{\emph{Intersection}} are common elements within the sets,
denoted by \(∩\) \[S \text{and} 𝑇 =𝑆∩𝑇=\{𝑏\} \] the notation \(𝑆 ∩ 𝑇\)
is called \(𝑆\) intersection \(𝑇\) which is equivalent to \(𝑆\)
𝑎𝑛𝑑\(𝑇\).

\textbf{\emph{Universal set}}: A set that contains every element denoted
\(𝓊\).

\textbf{\emph{Complement}} : all elements within a universal set THAT IS
NOT within the given set, denoted as 𝑛𝑜𝑡** \(𝐴 = 𝐴′ = 𝐴^𝐶 = \hd{𝐴}\) .

\textbf{\emph{Difference/Relative complement}} : these are elements in
the set \(𝑆\) BUT NOT in \(𝑇\) where \[S−𝑇=𝑆∩𝑇̅ =\{𝑎,𝑐\}\] There are also
addition, subtraction and multiplication for sets.
